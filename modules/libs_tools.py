...
def tool_konsep_rumah(penghuni, mobil, luas_tanah):
    """
    [TOOL ARSITEK] Membuat Program Ruang & Cek KDB otomatis.
    """
    arch = libs_arch.Architect_Engine()
    prog = arch.generate_program_ruang(penghuni, mobil, luas_tanah)
    
    # Format Laporan Singkat
    txt = f"ANALISIS PROGRAM RUANG (Standar Neufert):\n"
    txt += f"- Kebutuhan Luas Bangunan: {prog['Total_Luas_Bangunan']:.1f} m2\n"
    txt += f"- Status Regulasi: {prog['Status_KDB_60%']}\n\n"
    txt += "Rincian Ruang:\n"
    for idx, row in prog['Detail_Ruang'].iterrows():
        txt += f"- {row['Ruang']} ({row['Jml']} unit): {row['Luas']} m2\n"
        
    return txt

def tool_audit_green(luas_atap, hadap):
    """
    [TOOL GREEN] Cek potensi hemat air & orientasi.
    """
    eco = libs_green.Green_Audit()
    hujan = eco.hitung_panen_hujan(luas_atap, 2500) # Asumsi hujan 2500mm/thn
    matahari = eco.cek_orientasi_bangunan(hadap)
    
    return f"AUDIT GREEN DESIGN:\n1. {matahari}\n2. Potensi Panen Air Hujan: {hujan['Penghematan Harian']} ({hujan['Rekomendasi']})"
